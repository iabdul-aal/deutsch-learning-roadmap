...
import { 
  Play, CheckCircle2, Circle, Clock, Compass, 
  AlertTriangle, ExternalLink, Mic, PenTool, BookOpen, Flame 
} from 'lucide-react';

export const TodayDashboard = () => {
  const { 
    mode, setActiveView, completedTasks, toggleTask, 
    weakTopics, listeningMinutes, 
    speakingMinutes, writingTasksCompleted, streakDays 
  } = useApp();
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="paper-card p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-300 text-[11px] font-black uppercase">
              Day {currentDayNumber} of 56  |  Week {currentWeekNumber}
            </span>
            <span className="text-stone-500 text-xs font-mono"> |  Mode: {mode === 'intensive' ? 'Intensive (~5h/d)' : 'Standard (~3h/d)'}</span>
          </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded bg-amber-500 hover:bg-amber-400 text-stone-950 font-black text-xs transition-all shadow-sm"
          >
      ...
      {weakTopics.length > 0 && (
        <div className="p-4 rounded-lg bg-amber-50 border border-amber-300 text-amber-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded bg-amber-600 text-white font-extrabold text-xs shrink-0 hover:bg-amber-700"
          >
      ...
      <div className="paper-card p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-stone-200 pb-3">
          ...
          <span className="px-3 py-1 rounded bg-stone-100 text-amber-900 text-xs font-mono font-bold border border-stone-300">
          ...
          {tasksList.map((task, idx) => {
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded border flex items-start gap-3 transition-all cursor-pointer ${
                  isDone 
              ...
              >
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <h4 className={`text-xs font-bold ${isDone ? 'line-through text-stone-400' : 'text-stone-800'}`}>
                    {task.title}
                  </h4>
                </div>
                ...
                {task.link && (
                  <a
                    ...
                    className="px-2.5 py-1 rounded bg-stone-100 hover:bg-amber-500 hover:text-stone-950 text-amber-900 text-[11px] font-bold border border-stone-300 transition-all shrink-0 flex items-center gap-1"
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div 
          ...
          className="paper-interactive p-5 cursor-pointer space-y-2 group"
        >
